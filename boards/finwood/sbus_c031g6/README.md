...
| 22 | PA15 | NC |
| 23 | PB3 | NC |
| 24 | **PB4** | **Red error LED** (open-drain, active-low) |
| 25 | PB5 | NC (WKUP6 on C031Gx; unused) |
| 26 | PB6 | NC |
| 27 | **PB7** | **UART in** USART1_RX (AF0) |
| 28 | **PB8** | **Green activity LED** (push-pull, active-high) |
...
flowchart LR
  ...
  subgraph mcu["STM32C031G6U6 UFQFPN-28"]
    ...
    PB4["pin 24 PB4 red LED OD"]
    PB7["pin 27 PB7 USART1_RX"]
    PB8["pin 28 PB8 green LED"]
...
Green LED (activity)   ---- MCU pin 28 PB8  push-pull to LED cathode
                            (anode via ~330 Ω to 3V3), active-high

Red LED (error)        ---- 5V -- ~330 Ω -- LED anode
                            LED cathode -- MCU pin 24 PB4  open-drain, active-low
```
...
| 1 | D1 | Green activity LED + ~330 Ω to 3V3 on **PB8** |
| 1 | D2 | Red error LED + ~330 Ω from 5 V to **PB4** (open-drain) |
...
| `sbus-out` | USART2 TX | PA4 | 10 | 100000 8E2, `tx-invert`, TX-only, FIFO, IRQ 28 prio 0 |
| `led0` | GPIO | PB8 | 28 | push-pull active-high activity LED |
| `led1` | GPIO | PB4 | 24 | open-drain active-low error LED |
